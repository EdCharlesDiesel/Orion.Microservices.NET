...
namespace Orion.API.Person.Controllers
{
    ...
    [ApiController]
    public class ContactTypeController(IUnitOfWork unitOfWork) : ControllerBase
    {
        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            var versions = await unitOfWork.ContactTypes.GetAllAsync();
            return Ok(versions);
        }
        ...
        {
            var version = await unitOfWork.ContactTypes.GetByIdAsync(id);
            if (version == null) return NotFound();
            return Ok(version);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] DataAccess.Postgres.Entities.ContactType version)
        {
            ...
            try
            {
                await unitOfWork.ContactTypes.AddAsync(version);
                await unitOfWork.CompleteAsync();

                return CreatedAtAction(nameof(GetById), new { id = version.ContactTypeID }, version);
            }
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] DataAccess.Postgres.Entities.ContactType version)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                var existing = await unitOfWork.ContactTypes.GetByIdAsync(id);
                if (existing == null)
                {
                ...
                // existing.ContactTypeLine1 = version.ContactTypeLine1;
                // existing.ContactTypeLine2 = version.ContactTypeLine2;
                ...
                unitOfWork.ContactTypes.Update(existing);
                await unitOfWork.CompleteAsync();
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                unitOfWork.ContactTypes.Delete(existing);
                await unitOfWork.CompleteAsync();
