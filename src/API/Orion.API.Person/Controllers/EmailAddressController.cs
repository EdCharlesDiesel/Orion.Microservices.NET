...
namespace Orion.API.Person.Controllers
{
    ...
    [ApiController]
    public class EmailAddressController(IUnitOfWork unitOfWork) : ControllerBase
    {
        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            var versions = await unitOfWork.EmailAddresses.GetAllAsync();
            return Ok(versions);
        }
        ...
        {
            var version = await unitOfWork.EmailAddresses.GetByIdAsync(id);
            if (version == null) return NotFound();
            return Ok(version);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] DataAccess.Postgres.Entities.EmailAddress version)
        {
            ...
            try
            {
                await unitOfWork.EmailAddresses.AddAsync(version);
                await unitOfWork.CompleteAsync();

                return CreatedAtAction(nameof(GetById), new { id = version.EmailAddressID }, version);
            }
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] DataAccess.Postgres.Entities.EmailAddress version)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                var existing = await unitOfWork.EmailAddresses.GetByIdAsync(id);
                if (existing == null)
                {
                ...
                // existing.EmailAddressLine1 = version.EmailAddressLine1;
                // existing.EmailAddressLine2 = version.EmailAddressLine2;
                ...
                unitOfWork.EmailAddresses.Update(existing);
                await unitOfWork.CompleteAsync();
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                unitOfWork.EmailAddresses.Delete(existing);
                await unitOfWork.CompleteAsync();
