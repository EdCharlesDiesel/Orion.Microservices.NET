...
namespace Orion.API.Person.Controllers
{
    ...
    [ApiController]
    public class PhoneNumberTypeController(IUnitOfWork unitOfWork) : ControllerBase
    {
        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            var versions = await unitOfWork.PhoneNumberTypes.GetAllAsync();
            return Ok(versions);
        }
        ...
        {
            var version = await unitOfWork.PhoneNumberTypes.GetByIdAsync(id);
            if (version == null) return NotFound();
            return Ok(version);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] DataAccess.Postgres.Entities.PhoneNumberType version)
        {
            ...
            try
            {
                await unitOfWork.PhoneNumberTypes.AddAsync(version);
                await unitOfWork.CompleteAsync();

                return CreatedAtAction(nameof(GetById), new { id = version.PhoneNumberTypeID }, version);
            }
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] DataAccess.Postgres.Entities.PhoneNumberType version)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                var existing = await unitOfWork.PhoneNumberTypes.GetByIdAsync(id);
                if (existing == null)
                {
                ...
                // existing.PhoneNumberTypeLine1 = version.PhoneNumberTypeLine1;
                // existing.PhoneNumberTypeLine2 = version.PhoneNumberTypeLine2;
                ...
                unitOfWork.PhoneNumberTypes.Update(existing);
                await unitOfWork.CompleteAsync();
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                unitOfWork.PhoneNumberTypes.Delete(existing);
                await unitOfWork.CompleteAsync();
