...
namespace Orion.API.Person.Controllers
{
    ...
    [ApiController]
    public class StateProvinceController(IUnitOfWork unitOfWork) : ControllerBase
    {
        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            var versions = await unitOfWork.StateProvinces.GetAllAsync();
            return Ok(versions);
        }
        ...
        {
            var version = await unitOfWork.StateProvinces.GetByIdAsync(id);
            if (version == null) return NotFound();
            return Ok(version);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] DataAccess.Postgres.Entities.StateProvince version)
        {
            ...
            try
            {
                await unitOfWork.StateProvinces.AddAsync(version);
                await unitOfWork.CompleteAsync();

                return CreatedAtAction(nameof(GetById), new { id = version.StateProvinceID }, version);
            }
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] DataAccess.Postgres.Entities.StateProvince version)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                var existing = await unitOfWork.StateProvinces.GetByIdAsync(id);
                if (existing == null)
                {
                ...
                // existing.StateProvinceLine1 = version.StateProvinceLine1;
                // existing.StateProvinceLine2 = version.StateProvinceLine2;
                ...
                unitOfWork.StateProvinces.Update(existing);
                await unitOfWork.CompleteAsync();
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                unitOfWork.StateProvinces.Delete(existing);
                await unitOfWork.CompleteAsync();
