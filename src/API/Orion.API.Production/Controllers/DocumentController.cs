...
namespace Orion.API.Production.Controllers
{
    ...
    [ApiController]
    public class DocumentController(IUnitOfWork unitOfWork) : ControllerBase
    {
        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            var versions = await unitOfWork.Documents.GetAllAsync();
            return Ok(versions);
        }
        ...
        {
            var version = await unitOfWork.Documents.GetByIdAsync(id);
            if (version == null) return NotFound();
            return Ok(version);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] DataAccess.Postgres.Entities.Document version)
        {
            ...
            try
            {
                await unitOfWork.Documents.AddAsync(version);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] DataAccess.Postgres.Entities.Document version)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                var existing = await unitOfWork.Documents.GetByIdAsync(id);
                if (existing == null)
                {
                ...
                unitOfWork.Documents.Update(existing);
                await unitOfWork.CompleteAsync();
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                unitOfWork.Documents.Delete(existing);
                await unitOfWork.CompleteAsync();
