...
namespace Orion.API.Production.Controllers
{
    ...
    public class LocationController(IUnitOfWork unitOfWork) : ControllerBase
    {
        ...
        {
            var versions = await unitOfWork.Locations.GetAllAsync();
            return Ok(versions);
        }
        ...
        {
            var version = await unitOfWork.Locations.GetByIdAsync(id);
            if (version == null) return NotFound();
            return Ok(version);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] DataAccess.Postgres.Entities.Location version)
        {
            ...
            try
            {
                await unitOfWork.Locations.AddAsync(version);
                await unitOfWork.CompleteAsync();

                return CreatedAtAction(nameof(GetById), new { id = version.LocationID }, version);
            }
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] DataAccess.Postgres.Entities.Location version)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                var existing = await unitOfWork.Locations.GetByIdAsync(id);
                if (existing == null)
                {
                ...
                unitOfWork.Locations.Update(existing);
                await unitOfWork.CompleteAsync();
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                unitOfWork.Locations.Delete(existing);
                await unitOfWork.CompleteAsync();
