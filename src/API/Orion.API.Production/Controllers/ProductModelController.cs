...
namespace Orion.API.Production.Controllers
{
    ...
    [ApiController]
    public class ProductModelController(IUnitOfWork unitOfWork) : ControllerBase
    {
        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            var versions = await unitOfWork.ProductModels.GetAllAsync();
            return Ok(versions);
        }
        ...
        {
            var version = await unitOfWork.ProductModels.GetByIdAsync(id);
            if (version == null) return NotFound();
            return Ok(version);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] DataAccess.Postgres.Entities.ProductModel version)
        {
            ...
            try
            {
                await unitOfWork.ProductModels.AddAsync(version);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] DataAccess.Postgres.Entities.ProductModel version)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                var existing = await unitOfWork.ProductModels.GetByIdAsync(id);
                if (existing == null)
                {
                ...
                // existing.EmployeeProductModelHistories = new List<EmployeeProductModelHistory>();
                existing.ModifiedDate = version.ModifiedDate;

                unitOfWork.ProductModels.Update(existing);
                await unitOfWork.CompleteAsync();
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                unitOfWork.ProductModels.Delete(existing);
                await unitOfWork.CompleteAsync();
