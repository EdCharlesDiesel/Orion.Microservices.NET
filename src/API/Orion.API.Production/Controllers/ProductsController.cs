...
namespace Orion.API.Production.Controllers
{
    ...
    public class ProductsController(IUnitOfWork unitOfWork) : ControllerBase
    {
        ...
        {
            var versions = await unitOfWork.Products.GetAllAsync();
            return Ok(versions);
        }
        ...
        {
            var version = await unitOfWork.Products.GetByIdAsync(id);
            if (version == null) return NotFound();
            return Ok(version);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] DataAccess.Postgres.Entities.Product version)
        {
            ...
            try
            {
                await unitOfWork.Products.AddAsync(version);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] DataAccess.Postgres.Entities.Product version)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                ...
                unitOfWork.Products.Update(existing);
                await unitOfWork.CompleteAsync();
