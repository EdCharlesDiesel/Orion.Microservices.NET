...
namespace Orion.API.Production.Controllers
{
    ...
    [ApiController]
    public class TransactionHistoryController(IUnitOfWork unitOfWork) : ControllerBase
    {
        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            var versions = await unitOfWork.TransactionHistorys.GetAllAsync();
            return Ok(versions);
        }
        ...
        {
            var version = await unitOfWork.TransactionHistorys.GetByIdAsync(id);
            if (version == null) return NotFound();
            return Ok(version);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] DataAccess.Postgres.Entities.TransactionHistory version)
        {
            ...
            try
            {
                await unitOfWork.TransactionHistorys.AddAsync(version);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] DataAccess.Postgres.Entities.TransactionHistory version)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                var existing = await unitOfWork.TransactionHistorys.GetByIdAsync(id);
                if (existing == null)
                {
                ...
                // existing.EmployeeTransactionHistoryHistories = new List<EmployeeTransactionHistoryHistory>();
                existing.ModifiedDate = version.ModifiedDate;

                unitOfWork.TransactionHistorys.Update(existing);
                await unitOfWork.CompleteAsync();
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                unitOfWork.TransactionHistorys.Delete(existing);
                await unitOfWork.CompleteAsync();
