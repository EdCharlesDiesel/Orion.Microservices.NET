...
namespace Orion.API.Production.Controllers
{
    ...
    [ApiController]
    public class WorkOrderRoutingController(IUnitOfWork unitOfWork) : ControllerBase
    {
        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            var versions = await unitOfWork.WorkOrderRoutings.GetAllAsync();
            return Ok(versions);
        }
        ...
        {
            var version = await unitOfWork.WorkOrderRoutings.GetByIdAsync(id);
            if (version == null) return NotFound();
            return Ok(version);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] DataAccess.Postgres.Entities.WorkOrderRouting version)
        {
            ...
            try
            {
                await unitOfWork.WorkOrderRoutings.AddAsync(version);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] DataAccess.Postgres.Entities.WorkOrderRouting version)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                var existing = await unitOfWork.WorkOrderRoutings.GetByIdAsync(id);
                if (existing == null)
                {
                ...
                // existing.EmployeeWorkOrderRoutingHistories = new List<EmployeeWorkOrderRoutingHistory>();
                existing.ModifiedDate = version.ModifiedDate;

                unitOfWork.WorkOrderRoutings.Update(existing);
                await unitOfWork.CompleteAsync();
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                unitOfWork.WorkOrderRoutings.Delete(existing);
                await unitOfWork.CompleteAsync();
