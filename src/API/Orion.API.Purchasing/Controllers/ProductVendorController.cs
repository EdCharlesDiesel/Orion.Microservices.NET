...
namespace Orion.API.Purchasing.Controllers
{
    ...
    [ApiController]
    public class ProductVendorController(IUnitOfWork unitOfWork) : ControllerBase
    {
        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            var versions = await unitOfWork.ProductVendors.GetAllAsync();
            return Ok(versions);
        }
        ...
        {
            var version = await unitOfWork.ProductVendors.GetByIdAsync(id);
            if (version == null) return NotFound();
            return Ok(version);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] DataAccess.Postgres.Entities.ProductVendor version)
        {
            ...
            try
            {
                await unitOfWork.ProductVendors.AddAsync(version);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] DataAccess.Postgres.Entities.ProductVendor version)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                var existing = await unitOfWork.ProductVendors.GetByIdAsync(id);
                if (existing == null)
                {
                ...
                // existing.EmployeeProductVendorHistories = new List<EmployeeProductVendorHistory>();
                existing.ModifiedDate = version.ModifiedDate;

                unitOfWork.ProductVendors.Update(existing);
                await unitOfWork.CompleteAsync();
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                unitOfWork.ProductVendors.Delete(existing);
                await unitOfWork.CompleteAsync();
