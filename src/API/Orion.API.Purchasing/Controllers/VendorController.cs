...
namespace Orion.API.Purchasing.Controllers
{
    ...
    [ApiController]
    public class VendorController(IUnitOfWork unitOfWork) : ControllerBase
    {
        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            var versions = await unitOfWork.Vendors.GetAllAsync();
            return Ok(versions);
        }
        ...
        {
            var version = await unitOfWork.Vendors.GetByIdAsync(id);
            if (version == null) return NotFound();
            return Ok(version);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] DataAccess.Postgres.Entities.Vendor version)
        {
            ...
            try
            {
                await unitOfWork.Vendors.AddAsync(version);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] DataAccess.Postgres.Entities.Vendor version)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                var existing = await unitOfWork.Vendors.GetByIdAsync(id);
                if (existing == null)
                {
                ...
                // existing.EmployeeVendorHistories = new List<EmployeeVendorHistory>();
                existing.ModifiedDate = version.ModifiedDate;

                unitOfWork.Vendors.Update(existing);
                await unitOfWork.CompleteAsync();
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                unitOfWork.Vendors.Delete(existing);
                await unitOfWork.CompleteAsync();
