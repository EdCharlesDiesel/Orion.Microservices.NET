...
namespace Orion.API.Sales.Controllers
{
    ...
    public class CustomerController(IUnitOfWork unitOfWork) : ControllerBase
    {
        ...
        {
            var versions = await unitOfWork.Customers.GetAllAsync();
            return Ok(versions);
        }
        ...
        {
            var version = await unitOfWork.Customers.GetByIdAsync(id);
            if (version == null) return NotFound();
            return Ok(version);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] DataAccess.Postgres.Entities.Customer version)
        {
            ...
            try
            {
                await unitOfWork.Customers.AddAsync(version);
                await unitOfWork.CompleteAsync();

                return CreatedAtAction(nameof(GetById), new { id = version.CustomerID }, version);
            }
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] DataAccess.Postgres.Entities.Customer version)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                ...
                // existing.EmployeeCustomerHistories = new List<EmployeeCustomerHistory>();
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                unitOfWork.Customers.Delete(existing);
                await unitOfWork.CompleteAsync();
