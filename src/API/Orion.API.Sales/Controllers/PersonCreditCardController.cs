...
namespace Orion.API.Sales.Controllers
{
    ...
    [ApiController]
    public class PersonCreditCardController(IUnitOfWork unitOfWork) : ControllerBase
    {
        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            var versions = await unitOfWork.PersonCreditCards.GetAllAsync();
            return Ok(versions);
        }
        ...
        {
            var version = await unitOfWork.PersonCreditCards.GetByIdAsync(id);
            if (version == null) return NotFound();
            return Ok(version);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] DataAccess.Postgres.Entities.PersonCreditCard version)
        {
            ...
            try
            {
                await unitOfWork.PersonCreditCards.AddAsync(version);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] DataAccess.Postgres.Entities.PersonCreditCard version)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                var existing = await unitOfWork.PersonCreditCards.GetByIdAsync(id);
                if (existing == null)
                {
                ...
                // existing.EmployeePersonCreditCardHistories = new List<EmployeePersonCreditCardHistory>();
                existing.ModifiedDate = version.ModifiedDate;

                unitOfWork.PersonCreditCards.Update(existing);
                await unitOfWork.CompleteAsync();
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                unitOfWork.PersonCreditCards.Delete(existing);
                await unitOfWork.CompleteAsync();
