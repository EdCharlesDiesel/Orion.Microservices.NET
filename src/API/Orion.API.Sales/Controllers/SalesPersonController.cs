...
namespace Orion.API.Sales.Controllers
{
    ...
    [ApiController]
    public class SalesPersonController(IUnitOfWork unitOfWork) : ControllerBase
    {
        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            var versions = await unitOfWork.SalesPersons.GetAllAsync();
            return Ok(versions);
        }
        ...
        {
            var version = await unitOfWork.SalesPersons.GetByIdAsync(id);
            if (version == null) return NotFound();
            return Ok(version);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] DataAccess.Postgres.Entities.SalesPerson version)
        {
            ...
            try
            {
                await unitOfWork.SalesPersons.AddAsync(version);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, [FromBody] DataAccess.Postgres.Entities.SalesPerson version)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                var existing = await unitOfWork.SalesPersons.GetByIdAsync(id);
                if (existing == null)
                {
                ...
                // existing.EmployeeSalesPersonHistories = new List<EmployeeSalesPersonHistory>();
                existing.ModifiedDate = version.ModifiedDate;

                unitOfWork.SalesPersons.Update(existing);
                await unitOfWork.CompleteAsync();
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                unitOfWork.SalesPersons.Delete(existing);
                await unitOfWork.CompleteAsync();
