...
{
    /// <summary>
    /// Evaluates open forex positions and produces hedge recommendations.
    /// </summary>
    public sealed class HedgingEngine : IHedgingEngine
    {
        /// <inheritdoc />
        public Task<HedgingResult> AnalyzeAsync(
            HedgingRequest request,
            CancellationToken cancellationToken = default)
        {
            ArgumentNullException.ThrowIfNull(request);

            if (request.OpenPositions == null || request.OpenPositions.Count == 0)
            {
        ...
        private static List<HedgeRecommendation> BuildRecommendations(
            IEnumerable<HedgeCandidate>? candidates,
            decimal netExposure,
            decimal hedgeRequired,
            CancellationToken cancellationToken)
        {
            if (candidates == null || hedgeRequired <= 0m)
                return [];

            var hedgeDirection = netExposure > 0m ? "SHORT" : "LONG";
            ...
            return candidates
                ...
                .Select(candidate =>
                {
                    ...
                    {
                        Pair = candidate.Pair.Trim().ToUpperInvariant(),
        ...
        private static decimal CalculateHedgeRatio(HedgeCandidate candidate)
        {
            ...
            var volatilityAdjustment = candidate.VolatilityPercent <= 0m
                ? 1m
        ...
        private static string ResolveRiskSummary(
            decimal netExposure,
            decimal grossExposure,
            IReadOnlyCollection<HedgeRecommendation> recommendations)
        {
            var netExposurePercent = grossExposure > 0m
        ...
        private static string NormalizeDirection(string direction)
        {
            ...
            {
                "BUY" or "LONG" or "BULLISH" => "LONG",
                "SELL" or "SHORT" or "BEARISH" => "SHORT",
                _ => "UNKNOWN"
            };
        }
