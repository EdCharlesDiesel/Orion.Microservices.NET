...
namespace Orion.API.TradingEconomics.Handlers
{
    ...
    public class BuildPortfolioHandler    : IRequestHandler<BuildPortfolioCommand, List<PortfolioPosition>>
    {
        ...
        public async Task<List<PortfolioPosition>> Handle(BuildPortfolioCommand request,CancellationToken ct)
        {
            var signals = await _mediator.Send(new GenerateFxSignalsCommand());

            // Step 1: Filter strongest signals
            var filtered = signals
                .Where(x => x.SignalStrength > 0.5M)
            ...
            foreach (var s in selected)
            {
                ...
                positions.Add(new PortfolioPosition
                {
                    ...
                    SignalStrength = (decimal)s.SignalStrength,
            ...
            var totalInvVol = positions.Sum(p => 1.0M / (p.Volatility + 1e-6M));

            foreach (var p in positions)
            {
                p.Weight = 1.0M / (p.Volatility + 1e-6M) / totalInvVol;
            }
        ...
        private List<FxSignal> EnforceExposureLimits(List<FxSignal> signals,int maxPerCurrency)
        {
