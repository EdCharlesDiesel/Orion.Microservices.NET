...
namespace Orion.Core.MacroEngine.Application
{
        ...
        : IRequestHandler<GenerateFxSignalsCommand, List<FxSignal>>
    {
        ...
        public async Task<List<FxSignal>> Handle(
            GenerateFxSignalsCommand request,
            ...
            for (int i = 0; i < scores.Count; i++)
            {
                for (int j = i + 1; j < scores.Count; j++)
                {
                    ...
                    signals.Add(new FxSignal
                    {
                        ...
                        BaseScore = diff > 0 ? a.TotalScore : b.TotalScore,
        ...
        private double CalculateConfidence(double strength)
        {
